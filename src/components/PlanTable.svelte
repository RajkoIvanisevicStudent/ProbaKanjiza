<script>
	...
	let showLoading = true; // Indikator za prikazivanje Loading komponente
	...
	async function handleSubColumnsVisibility(desc, visible) {
		$WaitingLoadingGlobalVisible = true;
		console.log('Krece', $WaitingLoadingGlobalVisible);
		...
		$WaitingLoadingGlobalVisible = false;
	}
		...
		return new Promise(async (resolve, reject) => {
			for (const product of arrProducts) {
				...
				for (const value of arrSelectedProductValues) {
					tempObj[value.planPeriodId + '-valuePlanLY'] = value.valuePlanLY !== null ? value.valuePlanLY : '-';

					tempObj[value.planPeriodId + '-valueRealLY'] = value.valueRealLY !== null ? value.valueRealLY : '-';
					tempObj[value.planPeriodId + '-valuePlan'] = value.valuePlan !== null ? value.valuePlan : '-';
					tempObj[value.planPeriodId + '-valueReal'] = value.valueReal !== null ? value.valueReal : '-';
				}
			...
			let tableDataR = JSON.parse(JSON.stringify(tempTableData));
			// tableData = JSON.parse(JSON.stringify(tempTableData));

			tableData = transformJSON(tableDataR);
			// console.log('Ovo je tableData', tempTableData);
			console.log(tableData);
			// document.getElementById('Privrem').value = JSON.stringify(tableData);
			// document.getElementById('Privrem').select();
	...
	function transformJSON(originalData) {
		...
		for (const product of originalData) {
			...
			// OVO JE OD KLJUČNE VAŽNOSTI ZA SORTIRANJE, GRUPISANJE
			// Ranije Rajkovo rešenje:
			// let attributes = [{ prodAttrGrpName: 'Mesto ugradnje' }, { prodAttrGrpName: 'Rektifikacija' }];

			//let attributes = groupingAttributes[product.productClassId];
			...
			// function customSortJSONObjects2(arrToSort, sortingOrderArr, property) {
			// 	const indexMap = {};
			...
			// product.productAttributes = customSortJSONObjects(product.productAttributes, attributes, 'prodAttrGrpName');

			// Rajko kraj sortiranja

			// **********************************************************************************
			// V A Ž N O :  Naredni blok koda služi injektovanju klasa u stablo proizvoda.
			//              Skidanjem komentara, stablo će prikazivati i klase.
			// **********************************************************************************
			//
			...
			for (const attr of product.productAttributes) {
				...
				if (!existingNode) {
					...
					// existingNode = { 
					// 	productName: attr.prodAttrName, 
					// 	'total-valuePlanLY': attr.total-valuePlanLY, 
					// 	'total-valueRealLY': attr.total-valueReal,
					// 	'total-valuePlan' : attr.total-valuePlan,
					// 	'total-valueReal' : attr.total-valueReal,
					// 	[attrName]: attrValue, 
					// 	_children: [] 
					// };

					// RAJKO 24.10.2023. ZBOG SUME 
					// Initialize the result object with the properties from the first object
					existingNode = {...product[0]};
					// Delete the first property from the copy
					delete existingNode.id;

					existingNode.productName = attr.prodAttrGrpName;
					existingNode._children = [];
					
					// Loop through the remaining objects
					for (let i = 3; i < product.length; i++) {
						const currentObject = product[i];
						
						for (const prop in currentObject) {
							//  && typeof currentObject[prop] === 'number'
							if (currentObject.hasOwnProperty(prop)) {
								if (existingNode.hasOwnProperty(prop)) {
									if (typeof currentObject[prop] === 'number') {
										existingNode[prop] += currentObject[prop];
									} 
								} else {
									existingNode[prop] = currentObject[prop];
								}
				...
				// 	for (const prop in currentObject) {
				// 		if (currentObject.hasOwnProperty(prop) && typeof currentObject[prop] === 'number') {
				// 			if (resultObject.hasOwnProperty(prop)) {
				// 				resultObject[prop] += currentObject[prop];
				// 			} else {
				// 				resultObject[prop] = currentObject[prop];
				// 			}
				// 		}
	...
	<div class="NoPaddingModal">
		<Modal
			...
			on:close={() => {
				...
				// Bez ovoga, ćelija sa koje se pozvao modalni, ne dobija fokus, zbog čega navigacioni tasteri
				// primenjuju samu tabelu, a ne indikator fokusa ćelija
				showSumRedistributionModal = false;
				sumRedistributionSettings.cell.edit();
			}}
		>
